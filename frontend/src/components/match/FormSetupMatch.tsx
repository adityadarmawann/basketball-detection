...
export default function FormSetupMatch({ onComplete }: FormSetupMatchProps) {
  ...
  const handleRegionChange = (r: RegionChoice) => {
    setRegion(r)
    setTeamAId(''); setTeamBId(''); setCustomTeamA(''); setCustomTeamB('')
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTeamAName(teamAName); setTeamBName(teamBName); setRegionStore(regionName)
      setRoster(newRoster)
      onComplete()
    } catch (err: unknown) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-surface rounded-lg p-6 shadow-sm">
      ...
      {error && (
        <div className="mb-4 p-3 bg-danger/10 border border-danger rounded-lg text-danger text-sm">{error}</div>
      )}

      {/* ── Region ── */}
      <div className="mb-6">
        <label className="block text-text-secondary text-sm font-medium mb-2">Region</label>
        <select value={region} onChange={(e) => handleRegionChange(e.target.value as RegionChoice)}
        ...
        {region === 'Lainnya' && (
          <input type="text" value={customRegion} onChange={(e) => setCustomRegion(e.target.value)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div>
          ...
          <select value={teamAId} onChange={(e) => setTeamAId(e.target.value)} disabled={!region || loading}
          ...
          {teamAId === 'custom' && (
            <input type="text" value={customTeamA} onChange={(e) => setCustomTeamA(e.target.value)}
        ...
        <div>
          ...
          <select value={teamBId} onChange={(e) => setTeamBId(e.target.value)} disabled={!region || loading}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm disabled:opacity-50 disabled:cursor-not-allowed">
          ...
          {teamBId === 'custom' && (
            <input type="text" value={customTeamB} onChange={(e) => setCustomTeamB(e.target.value)}
        ...
        <div>
          ...
          <select value={round} onChange={(e) => setRound(e.target.value)} disabled={loading}
            className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary text-sm disabled:opacity-50">
            <option>Final</option>
