...
export const useVideoUpload = () => {
  ...
  const uploadVideo = async (file: File, quarter?: number) => {
    ...
    try {
      ...
      // Only send jersey colors when user explicitly set them. Empty strings
      // tell backend to use K-Means clustering instead of preset colors.
      formData.append('jersey_color_a', jerseyColorA || '')
      formData.append('jersey_color_b', jerseyColorB || '')
